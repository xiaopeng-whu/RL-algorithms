...
'''
    Noisy_net DQN的想法很简单，它在训练中学习探索的方式，而不是使用固定的探索策略（如ε-greedy），将全连接层的权重w和b引入噪声
    即：y = wx+b  ——→  y = (μ_w+σ_w·ε_w)x+μ_b+σ_b·ε_b
    有两种添加噪声的方式：Independent Gaussian noise/Factorized Gaussian noise （两种结果根据书中所说没有明显区别）
    这里实现第一种，参考 https://github.com/PacktPublishing/Deep-Reinforcement-Learning-Hands-On/blob/master/Chapter07/lib/dqn_model.py
'''
class NoisyLinear(nn.Linear):
    def __init__(self, in_features, out_features, sigma_init=0.017, bias=True):
        super(NoisyLinear, self).__init__(in_features, out_features, bias=bias)
        self.sigma_weight = nn.Parameter(torch.full((out_features, in_features), sigma_init))   # 为了σ可训练，使用nn.Parameter生成Tensor
        self.register_buffer("epsilon_weight", torch.zeros(out_features, in_features))  # register_buffer在网络中创建一个张量，在反向传播期间不会被更新
    ...
    def reset_parameters(self):     # 按照文章中的建议进行初始化
...
class NoisyDQN(nn.Module):
    def __init__(self, in_channels, n_actions):
        super(NoisyDQN, self).__init__()     # 子类Network继承父类nn.Module的所有属性和方法，并用父类方法初始化
...
class Agent:
    def __init__(self, env, device):
        ...
        self.policy_net = NoisyDQN(in_channels=env.observation_space.shape[2], n_actions=env.action_space.n)
        self.target_net = NoisyDQN(in_channels=env.observation_space.shape[2], n_actions=env.action_space.n)
