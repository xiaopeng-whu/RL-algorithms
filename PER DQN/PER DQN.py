...
'''
    参考：https://github.com/Sntz91/dqn_multistep_experiments/blob/main/lib/multistep_replay_memory.py
    https://github.com/PacktPublishing/Deep-Reinforcement-Learning-Hands-On/blob/master/Chapter07/05_dqn_prio_replay.py
    PERBuffer:
        1.the priority of every sample in the buffer is calculated as P(i)=pi^α/Σpk^α (stochastic prioritization)为了总是选TD-error大的，防止overfit
            probability to the power of alpha to weight how important that probability it, 0 = normal distirbution
            pi = σi + ε,  σi is TD-error (greedy TD-error prioritization) 
        2.New samples added to the buffer need to be assigned a maximum value of priority to be sure that they will be sampled soon.
        3.Prioritized replay introduces bias（数据漂移） because it doesn't sample experiences uniformly at random due to the sampling proportion correspoding to TD-error. 
            We can correct this bias by using importance-sampling (IS) weights：wi=(N*P(i))^(-β)
            DQN中引入经验池是为了解决数据相关性，使数据（尽量）独立同分布的问题。但是使用优先经验回放又改变了状态的分布，这样势必会引入偏差bias
            使用SGD更新就必须对其进行补偿，引入importance-sample weights来弥补。β=1则完全补偿。
'''
class PERBuffer:
    ...
    def sample_v1(self, batch_size):
        ...
        indexs = np.random.choice(len(self.buffer), batch_size, p=probs)
        ...
        weights = (total * probs[indexs]) ** (-self.beta)
        weights /= weights.max()
        ...
        return np.array(states), np.array(actions), np.array(rewards, dtype=np.float32), np.array(dones, dtype=np.bool), np.array(next_states), \
               indexs, np.array(weights, dtype=np.float32)
...
class Agent:
    def __init__(self, env, device):
        ...
        self.buffer = PERBuffer(capacity=self.buffer_capacity, prob_alpha=0.6, beta_start=0.4, beta_frames=100000)
    ...
    def optimize_model(self, device="cpu"):
        ...
        states, actions, rewards, dones, next_states, batch_indices, batch_weights = self.buffer.sample_v1(self.batch_size)
        ...
        # calculate dqn loss
        # 由于PyTorch中的MSELoss类不支持权值(这是可以理解的，因为MSE是回归问题中使用的损失，但在分类损失中通常使用的是样本的权重)，我们需要计算MSE，并将结果与权值明确相乘:
        # loss = F.smooth_l1_loss(current_q_value, target_value)
        losses_v = batch_weights_v * (current_q_value - target_value) ** 2
        loss = losses_v.mean()
