...
class DuelingDQN(nn.Module):
    def __init__(self, in_channels, n_actions):
        super(DuelingDQN, self).__init__()     # 子类Network继承父类nn.Module的所有属性和方法，并用父类方法初始化
        self.conv = nn.Sequential(  # V网络和A网络前面都连接一个特征提取的CNN网络，相当于共享参数
    ...
    def forward(self, x):   # 接受4D的输入tensor:(batch_size, color_channel, height, width)
        ...
        return val + (adv - adv.mean(dim=1, keepdim=True))  # Q = V + (A - 1/|A|ΣA)
...
class Agent:
    def __init__(self, env, device):
        ...
        self.policy_net = DuelingDQN(in_channels=env.observation_space.shape[2], n_actions=env.action_space.n)
        self.target_net = DuelingDQN(in_channels=env.observation_space.shape[2], n_actions=env.action_space.n)
    ...
    def optimize_model(self, device="cpu"):
        ...
        '''
            Double DQN 和 DQN 的主要区别就在这里：
                Nature DQN在s'状态选择动作和对动作求值都使用target网络
                DDQN选择使用policy网络、求值使用target网络，以缓解最大化造成的高估
        '''
        next_policy_q_value = self.policy_net(next_states_batch.float())
        next_target_q_value = self.target_net(next_states_batch.float())
        # print("next_policy_q_value:", next_policy_q_value)
        a = torch.max(next_policy_q_value, 1)[1].unsqueeze(1)
        # print("action_max_index:", a)
        next_q_value = next_target_q_value.gather(1, a).squeeze()
