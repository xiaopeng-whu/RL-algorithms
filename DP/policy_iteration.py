...
def policy_evaluation(env, policy, V, discount_factor):
    """
    ...
    for state, action in enumerate(policy):
        for probability, next_state, reward, info in env.P[state][action]:
            policy_value[state] += probability * (reward + (discount_factor * V[next_state]))
...
def policy_iteration(env, discount_factor=0.999, max_iteration=1000):
    ...
    V = np.zeros(env.observation_space.n)    # env.nS = env.observation_space.n
    ...
    for i in range(max_iteration):
        ...
        V = policy_evaluation(env, policy, V, discount_factor)
        # improve policy
        policy = policy_improvement(env, policy, V, discount_factor)
        # if policy not changed over 10 iterations it converged.
        if i % 10 == 0:
            if np.all(np.equal(policy, policy_prev)):   # 判断两个numpy数组是否相等
...
if __name__ == "__main__":
    ...
    tic = time.time()
    opt_V, opt_policy = policy_iteration(environment.env, discount_factor=0.999, max_iteration=10000)
    toc = time.time()
    ...
    print(f'Total win_times with Policy iteration: {win_times}')
